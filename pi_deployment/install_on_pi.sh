...
echo "[3/3] Removing old background services and setting up launcher..."
if systemctl list-unit-files | grep -q "fantasy-backend.service"; then
...
if systemctl list-unit-files | grep -q "fantasy-frontend.service"; then
...
# Fix Windows CRLF line endings which break bash scripts and desktop entries
sed -i 's/\r$//' "$LAUNCHER_FILE"
...
echo " Old background services have been removed."
echo " To view the dashboard, tap the 'Fantasy Dashboard' icon on your desktop."
echo " It will boot the servers, open the UI, and cleanly shut them down when you exit!"
echo "================================================="
